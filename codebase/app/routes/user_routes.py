...
async def list_users(skip: int = 0, limit: int = 100, db: Session = Depends(PostgresClient.db), current_user: schemas.UserRead = Depends(AuthService.get_current_user)):
...
async def read_user(user_id: uid.UUID, db: Session = Depends(PostgresClient.db), current_user: schemas.UserRead = Depends(AuthService.get_current_user)):
...
async def update_user(user_id: uid.UUID, user_update: schemas.UserUpdate, db: Session = Depends(PostgresClient.db), current_user: schemas.UserRead = Depends(AuthService.get_current_user)):
...
async def delete_user(user_id: uid.UUID, db: Session = Depends(PostgresClient.db), current_user: schemas.UserRead = Depends(AuthService.get_current_user)):
...
async def list_projects_by_user_id(user_id: uid.UUID, skip: int = 0, limit: int = 100, db: Session = Depends(PostgresClient.db), current_user: schemas.UserRead = Depends(AuthService.get_current_user)):
    projects = crud.project.list_by_foreign_key(db, "user_id", user_id, skip=skip, limit=limit, current_user=current_user)
    count = crud.project.count_by_foreign_key(db, "user_id", user_id, current_user=current_user)
