...
def test_update_project(auth_client, project):
    ...
    response = auth_client.put(
        ...
        json={"title": "Updated Project Title", "description": "Updated description"},
    )
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == project_id
...
def test_update_nonexistent_project(auth_client):
    non_existent_id = "00000000-0000-0000-0000-000000000000"
    response = auth_client.put(
        f"/projects/{non_existent_id}",
        json={"title": "Should Not Work", "description": "This project does not exist"},
    )
    assert response.status_code == 404
